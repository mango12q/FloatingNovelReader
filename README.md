...
| 版本 | 说明 |
|------|------|
| **v0.8** | 字体中文名置顶（未安装置灰）、≡ 菜单添加书签、背景色精简五种预设、86 用例 |
| v0.7 | 分页引擎 TextFormatter 重写、进度/编码链路修复、双打包产物、安装免打扰、系统字体识别、83 用例 |
| v0.6 | 根据用户建议初步优化：书架窗口改为每次新建实例（修复托盘重复打开崩溃）等 |
| v0.5 | 修复 UTF-16/GBK 编码 TXT 导入乱码，章节识别补齐「第 N 部」与楔子/番外 |
